...
class TelescopeVisibilityModel:
    ...
    def apply_corrections(
        ...
        # Terapkan koreksi
        # Faktor FB dan FI adalah faktor REDUKSI (nilai < 1 untuk teleskop yang baik)
        # - FB menggabungkan: Fb*Ft*Fp*Fa*Fm (termasuk Fm = 1/M² yang sangat kecil)
        # - FI menggabungkan: Fb*Ft*Fp*Fa*Fr
        # 
        # Rumus yang BENAR: PERKALIAN (bukan pembagian)
        # - B_eff = B_0 * FB → sky menjadi lebih gelap (FB < 1)
        # - I_eff = I_0 * FI → hilal juga lebih gelap tapi tidak sebanyak sky
        B_eff = B_0 * FB
        I_eff = I_0 * FI
...
def calculate_telescope_visibility(
    ...
    age : float
        Usia pengamat (tahun), default 30
    seeing : float
...
if __name__ == "__main__":
    ...
    age_val = 30       # Usia pengamat 30 tahun
    t1_val = 0.95      # Transmisi per lensa/cermin 95%
    n_val = 4          # 4 permukaan optik (2 lensa objektif + 2 eyepiece)

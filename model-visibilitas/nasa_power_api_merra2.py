...
def _fetch_power_hourly(day_utc: date, location: ObservingLocation) -> List[Dict[str, str]]:
    ...
    The request asks for the ``RH2M`` and ``T2M`` parameters for the
    ...
    list of dict
        ...
        ``RH2M``, and ``T2M``.  The values are strings that must be
...
def get_rh_t_at_time(location: ObservingLocation, target_utc: datetime) -> Tuple[float, float]:
    ...
    ``RH2M`` and ``T2M`` values linearly between the two surrounding
    ...
    target_utc : datetime
        ...
        (e.g. sunset) at which RH and temperature are needed.  The
    ...
    if rh0 is None or rh1 is None or t2m0 is None or t2m1 is None:
        raise PowerAPIError(
            f"Missing RH2M/T2M data for interpolation around {target_utc.isoformat()}"
        )

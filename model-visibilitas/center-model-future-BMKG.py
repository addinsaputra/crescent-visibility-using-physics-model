"""
Program Prediksi Visibilitas Hilal dengan Data Prakiraan BMKG
==============================================================

Program ini mengintegrasikan model Schaefer (untuk sky brightness), model Kastner
(untuk luminansi hilal), dan data prakiraan cuaca dari BMKG API untuk memprediksi
visibilitas hilal MASA DEPAN (3 hari ke depan) dengan otomatisasi penuh.

Perbedaan dengan versi historis (ERA5/MERRA-2):
- Menggunakan data PRAKIRAAN dari BMKG (bukan data historis)
- Cocok untuk prediksi visibilitas hilal mendatang
- Data tersedia 3 hari ke depan dengan resolusi 3 jam
- Menggunakan kode wilayah administrasi (adm4) BMKG

Alur Program:
1. Input user: bulan hijriah, tahun hijriah, lokasi (dengan kode adm4 BMKG)
2. Otomatisasi: hitung waktu ijtima/konjungsi
3. Otomatisasi: hitung sunset dengan koreksi bertahap:
   a. Sunset GEOMETRIS (tanpa koreksi refraksi)
   b. Ambil data prakiraan atmosfer (RH, T) dari BMKG API
...
6. Hitung sky brightness menggunakan model Schaefer
7. Hitung luminansi hilal menggunakan model Kastner
...
- Forecast period: 3 hari ke depan
- Data resolution: Setiap 3 jam (8 data per hari)
- Update frequency: 2 kali sehari
- Rate limit: 60 requests per menit per IP

Variabel yang digunakan:
- t: Suhu udara (°C)
- hu: Kelembapan udara (%)
- utc_datetime: Waktu dalam UTC

Catatan Penting:
- Program menggunakan kode wilayah administrasi tingkat IV (kelurahan/desa)
- Lokasi predefined tersedia di bmkg_api_forecast.py
- Data prakiraan hanya tersedia untuk 3 hari ke depan
- Untuk data historis, gunakan center-model-historis-era5.py atau center-model-historis-merra2.py

Author: Droid AI Assistant
Date: 2026-01-17
Version: 1.0 (BMKG API untuk Prakiraan Cuaca)
"""
...
# Import modul prakiraan cuaca BMKG
from bmkg_api_forecast import (
    ObservingLocation,
    get_rh_t_at_time,
    BMKGAPIError,
...
class HilalVisibilityCalculator:
    """Kelas utama untuk kalkulasi visibilitas hilal dengan data prakiraan BMKG"""
    ...
    def __init__(self,
        ...
        """
        Inisialisasi kalkulator visibilitas hilal dengan BMKG API.
        ...
        tahun_hijri : int
            Tahun hijriah
        adm4_code : str
            Kode wilayah administrasi BMKG tingkat IV (kelurahan/desa).
            Format: "xx.xx.xx.xxxx" (provinsi.kabkota.kecamatan.kelurahan)
            Contoh: "33.74.10.1003" untuk Kelurahan Ngaliyan, Semarang
    ...
    def tentukan_tanggal_pengamatan(self, ijtima_utc: datetime) -> Tuple[datetime, datetime, float, float]:
        ...
        # Simpan juga UTC untuk keperluan lain
        sunset_geom_utc = convert_localtime_to_utc(self.timezone_str, local_datetime=sunset_geom_local)

        # LANGKAH 2: Ambil data prakiraan atmosfer (RH dan T) dari BMKG API
        # dengan interpolasi linear ke waktu sunset LOKAL
        # (BMKG menggunakan waktu lokal Indonesia, tidak perlu konversi UTC)
        ...
        # Gunakan waktu lokal langsung (BMKG menyediakan data dalam waktu lokal)
        # Pastikan sunset_geom_local adalah naive datetime (tanpa timezone)
        sunset_local_naive = sunset_geom_local.replace(tzinfo=None) if sunset_geom_local.tzinfo else sunset_geom_local

        try:
            rh, temperature = get_rh_t_at_time(loc, sunset_local_naive)
        except (BMKGAPIError, Exception) as e:
            print(f"  [!] BMKG API Error: {e} - Menggunakan default RH=80%, T=25°C")
            rh = 80.0
            temperature = 25.0
    ...
    def ambil_data_atmosfer(self, sunset_local: datetime) -> Tuple[float, float]:
        """
        Mengambil data prakiraan atmosfer (RH dan suhu) dari BMKG API dengan interpolasi linear.

        BMKG menggunakan waktu lokal Indonesia, jadi fungsi ini menerima waktu lokal langsung.

        Parameters:
        -----------
        sunset_local : datetime
            Waktu sunset dalam waktu LOKAL (bukan UTC)
        ...
        if sunset_local is None:
            raise ValueError("sunset_local tidak boleh None")
        
        # Pastikan sunset_local adalah naive datetime (tanpa timezone)
        sunset_local_naive = sunset_local.replace(tzinfo=None) if sunset_local.tzinfo else sunset_local
...
def main():
    ...
    print("\n" + "=" * 70)
    print("CATATAN: Data prakiraan BMKG hanya tersedia 3 hari ke depan.")
    print("Untuk data historis, gunakan center-model-historis-era5.py atau")
    print("center-model-historis-merra2.py")
    print("=" * 70)

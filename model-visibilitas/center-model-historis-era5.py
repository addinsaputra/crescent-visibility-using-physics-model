...
(untuk luminansi hilal), dan data atmosferik dari Open-Meteo ERA5 API untuk
memprediksi visibilitas hilal dengan otomatisasi penuh.
...
3. Otomatisasi: hitung sunset dengan koreksi bertahap:
   ...
   b. Ambil data atmosfer (RH, T) dari Open-Meteo ERA5 API pada waktu sunset geometris
...
============================================================
PERBAIKAN PENTING - Versi 2026-01-17 (Update 7)
============================================================

1. GANTI DATA ATMOSFER DARI NASA POWER API KE OPEN-METEO ERA5 API
   ----------------------------------------------------------------
   Perubahan: Mengganti sumber data atmosfer dari NASA POWER (MERRA-2) ke Open-Meteo ERA5

   Keuntungan:
   - ERA5 adalah reanalisis modern dengan resolusi lebih tinggi (31km)
   - Data historis tersedia dari 1940 hingga sekarang
   - API Open-Meteo lebih stabil dan tidak memerlukan authentication
   - Respon lebih cepat
   - Data hourly yang lebih akurat

   Variabel ERA5 yang digunakan:
   - temperature_2m: Suhu udara 2 meter di atas permukaan (°C)
   - relative_humidity_2m: Kelembapan relatif 2 meter di atas permukaan (%)

   Perubahan kode:
   - Import: from open_meteo_api_era5 import fetch_hourly_weather, fetch_weather_with_info
   - Fungsi: fetch_hourly_weather() mengembalikan DataFrame dengan data hourly
   - Interpolasi: Mencari data hourly yang paling dekat dengan waktu sunset
...
1. ALUR KOREKSI SUNSET YANG BENAR (GEOMETRIS → ATMOSFER → REFRAKSI)
   ...
   b. Ambil data atmosfer (RH, T) dari Open-Meteo ERA5 API
      - Waktu pengambilan data: pada saat sunset geometris
      - Data yang diambil: Relative Humidity (RH) dan Temperature (T)
      - Fungsi: fetch_hourly_weather() dari open_meteo_api_era5.py
...
2. FUNGSI BARU DI sunmoon.py
   ...
   b. sunrise_sunset_apparent_local(location, timezone, year, month, day, T, P)
      ...
      - Parameter T (temperature) diambil dari Open-Meteo ERA5 API
...
3. OTOMATISASI DATA ATMOSFER
   ---------------------------
   Peningkatan: Data atmosfer (RH dan T) diambil secara otomatis dari Open-Meteo ERA5 API
...
- Temperature untuk koreksi refraksi diambil otomatis dari Open-Meteo ERA5 API
- Pressure di-set statis (default: 1013.25 mbar)
...
Date: 2026-01-17
Version: 7.0 (Open-Meteo ERA5 API untuk Data Atmosfer)
"""
...
from open_meteo_api_era5 import ObservingLocation, get_rh_t_at_time, ERA5APIError
...
class HilalVisibilityCalculator:
    ...
    def tentukan_tanggal_pengamatan(self, ijtima_utc: datetime) -> Tuple[datetime, datetime, float, float]:
        ...
        # Menggunakan Open-Meteo ERA5 API dengan interpolasi linear
        ...
        except (ERA5APIError, Exception) as e:
            print(f"  [!] ERA5 API Error: {e} - Menggunakan default RH=80%, T=25°C")
            rh = 80.0
            temperature = 25.0
    ...
    def ambil_data_atmosfer(self, sunset_utc: datetime) -> Tuple[float, float]:
        """
        Mengambil data atmosfer (RH dan suhu) dari Open-Meteo ERA5 API dengan interpolasi linear.
